   ...
   try {
      ...
      const res = await client.query(
         `SELECT
            ...
            concat(c.udt_name,
            case when coalesce(case when c.numeric_precision is not null then (case when c.numeric_scale > 0 then concat('(',c.numeric_precision,',',c.numeric_scale,')') else concat('(',c.numeric_precision,')') end) else null end, concat('(',c.character_maximum_length,')')) = '()'
            then ''
            else coalesce(case when c.numeric_precision is not null then (case when c.numeric_scale > 0 then concat('(',c.numeric_precision,',',c.numeric_scale,')') else concat('(',c.numeric_precision,')') end) else null end, concat('(',c.character_maximum_length,')'))
            end) as "Type",
            c.is_nullable as "Null?",
            c.column_default as "Default",
            replace(pgd.description,'
','\n') as "Comment"

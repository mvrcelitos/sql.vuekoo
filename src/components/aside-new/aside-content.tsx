...
export const AsideContent = ({ databases, isTablet, ...props }: AsideContentProps) => {
   ...
   return (
      <AnimatePresence custom={selected}>
         {selected ? (
            <motion.aside
               initial={{ opacity: 0, width: 0, borderRightWidth: 0 }}
               animate={{ opacity: 1, width: isTablet ? "100%" : 304, borderRightWidth: 1 }}
               exit={{ opacity: 0, width: 0, borderRightWidth: 0, transition: { borderRightWidth: { duration: 0 } } }}
               transition={{ type: "spring", duration: 0.5, bounce: 0.15 }}
               className={cn(
                  "modern-scroll relative z-[31] flex min-h-[50svh] flex-col overflow-y-auto overflow-x-hidden border-r-muted bg-background",
               )}>
